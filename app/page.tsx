    ...
    <div className="min-h-screen bg-gray-50">
      <div className="max-w-6xl mx-auto px-4 py-6">
        ...
        <div className="bg-white rounded-2xl shadow-sm p-6 mb-6">
          <h2 className="text-lg font-semibold text-gray-800 mb-4">投稿ライブラリ</h2>
        ...
        {/* Staff Picks Section */}
        <div className="bg-white rounded-2xl shadow-sm p-6 mb-6">
          <h2 className="text-lg font-semibold text-gray-800 mb-4">運営ピックアップ</h2>
            ...
            <div className="grid md:grid-cols-3 gap-4">
              {staffPicks.creators.map((creator) => (
                <div key={creator.id} className="text-center">
                  ...
                  <h3 className="font-medium text-gray-800 mb-1">{creator.name}</h3>
        ...
        {/* Recommended Genres */}
        <div className="bg-white rounded-2xl shadow-sm p-6 mb-6">
          <h2 className="text-lg font-semibold text-gray-800 mb-4">おすすめのジャンル</h2>
          <div className="flex flex-wrap gap-2 mb-4">
            {genres.map((genre) => (
              <button
                key={genre}
                className="px-4 py-2 bg-gray-100 text-gray-700 rounded-full text-sm hover:bg-pink-100 hover:text-pink-600 transition-colors"
              >
                {genre}
              </button>
            ))}
        ...
        <div className="grid md:grid-cols-2 gap-6">
          ...
          <div className="bg-white rounded-2xl shadow-sm p-6">
            <h2 className="text-lg font-semibold text-gray-800 mb-4">ランキング - 投稿</h2>
              ...
              {rankingPosts.map((post) => (
                <div key={post.id} className="flex items-center gap-3">
                  ...
                  <div className="flex-1">
                    <h3 className="font-medium text-gray-800 text-sm">{post.title}</h3>
          ...
          {/* Creator Ranking */}
          <div className="bg-white rounded-2xl shadow-sm p-6">
            <h2 className="text-lg font-semibold text-gray-800 mb-4">ランキング - クリエイター</h2>
            <div className="space-y-3">
              {rankingCreators.map((creator) => (
                <div key={creator.id} className="flex items-center gap-3">
                  ...
                  />
                  <div className="flex-1">
                    <h3 className="font-medium text-gray-800 text-sm">{creator.name}</h3>
                    <p className="text-xs text-gray-500">{creator.subscribers.toLocaleString()}人</p>
                  </div>
                </div>
              ))}
            </div>
